...
from zhunter.colors import ColorManager
...
class MainGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Define the self.config dictionary with all the loaded
    ...
    def connect_actions(self):
        ...
        self.actionWavelength_correction.triggered.connect(
            self.windows["wvlg_corr"].show
        )
    ...
    def show_hide_uncertainty(self):
        ...
        self.GraphicWidget.show_hide_uncertainty(
            show=self.show_uncertainty_chb.isChecked()
        )
    ...
    def show_hide_fine_structure(self):
        ...
        if indexes:
            ...
            self.specsysModel.show_hide_fine_structure(
                index,
                bounds=[self.data.values["wvlg_min"], self.data.values["wvlg_max"]],
            )
        else:
    ...
    def set_spat_ext_width(self):
        ...
        if self.mode == "2D":
            try:
                ...
                self.GraphicWidget.roi.setSize(
                    [self.data.values["wvlg_span"].value, ext_width]
                )
            except ValueError:
                QtWidgets.QMessageBox.information(
                    self,
    ...
    def find_ratio_names(self):
        ...
        try:
            ...
            mask = (
                abs(self.config["lines"]["ratios"]["ratio"] - ratio_value)
                <= ratio_error_margin
            )
    ...
    def add_specsys(self, z, sys_type="abs"):
        ...
        try:
            if sys_type == "abs":
                ...
                lines = join(
                    self.config["lines"]["intervening"],
                    self.config["lines"]["GRB"],
                    join_type="outer",
                )
            elif sys_type == "em":
